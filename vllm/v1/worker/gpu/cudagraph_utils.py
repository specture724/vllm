...
class CudaGraphManager:
    ...
    def _capture_stream(
        ...
        """Stream to capture `desc` on, or None for `torch.cuda.graph`'s own.
...
class ModelCudaGraphManager(CudaGraphManager):
    ...
    def capture(
        ...
        def create_forward_fn(
            ...
            if desc.num_ubatches > 1:
                # Split capture, FULL only (PIECEWISE+DBO is out of scope, as
                # in V1). Build the dummy microbatches through the same
                # `UBatchRunner.prepare` real steps use, then start the
                # microbatch threads now -- outside any graph, on
                # `ubatch_runner.capture_stream`, the stream `capture()` opens
                # the graph on -- and defer only the handoff-and-join to the
                # returned forward_fn. Exactly like V1's
                # `gpu_ubatch_wrapper.py::_capture_ubatches`: the two threads'
                # compute/comm stream ops are what actually gets captured,
                # while thread setup and the barrier wait (pure CPU/event
                # synchronization) stay off the captured stream.
                assert self.ubatch_runner is not None
